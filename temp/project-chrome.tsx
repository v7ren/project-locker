...
const dockPillInnerSolid =
  "pointer-events-auto flex flex-wrap gap-1.5 rounded-2xl border border-zinc-200 bg-zinc-50 px-2 py-1.5 text-xs shadow-md dark:border-zinc-600 dark:bg-zinc-950";
...
function BottomRightProjectDock({ slug }: { slug: string }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          onHome ? dockPillInnerSolid : dockPillInnerGlass,
      ...
      >
        <div className="flex flex-wrap justify-end gap-1.5">
          <Link
            href={`/${slug}/dashboard`}
            className={cn(
              "rounded-lg px-2.5 py-1 font-medium text-zinc-800 dark:text-zinc-100",
              onHome
...
function ProjectBreadcrumbLayer({ slug, name }: Props) {
  ...
  if (onHome) {
    return <CenterBreadcrumbBar items={homeItems} solidChrome />;
  }
